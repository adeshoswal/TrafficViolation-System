...
def process(src):
    ...
    cv2.imwrite("3_detected_plate.png", src)
    adjusted, a, b = automatic_brightness_and_contrast(src)
    cv2.imwrite("4_Brigthness_contrast_adjustment.png", adjusted)
    # BGR to gray
    gray = cv2.cvtColor(adjusted, cv2.COLOR_BGR2GRAY)
    cv2.imwrite("5_gray.png", gray)
    # Binary thresh
    #ret, th = cv2.threshold(gray, 140, 255, cv2.THRESH_BINARY)
    ret, th = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    cv2.imwrite("6_threshold.png", th)
    return th
